...
export const verifyCreateProjectParams = async (ctx: Context, next: Next) => {
  const { name, status, startAt } = ctx.request.body as Partial<CreateProjectParams>
  if (!name) return throwError(ctx, 'Name is required!', 400)
  if (status && !['pending', 'doing', 'done'].includes(status)) return throwError(ctx, 'Status Is Invalid!', 400)
